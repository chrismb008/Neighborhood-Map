...
class FilterLocations extends Component {
	...
	updateQuery = (query) => {
		/* Update the visible query manage the sync of the different state arrays */
		this.setState({
			query,
			listIsOpen: true
		});
	...
	manageClickedMarker = (location) => {
		/* Manage the animation of the markers when clicking on the list item */
		let controlledThis = this;
		...
		/* Get the current marker. Open the InfoWinow accordingly */
	...
	getCurrentMarker = (location) => {
		/* Get the marker clicked to give the good info in the InfoWindow */
		this.state.filteredMarkers.map(filteredMarker =>
			filteredMarker.id === location.key &&
				this.setState({
